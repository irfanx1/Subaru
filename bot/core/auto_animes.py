...
async def mirror_to_main_channel(post_msg, photo_url, caption, channel_to_use):
    try:
        ...
        # ── Build link button to side channel (channel link, not post link) ──
        buttons = None
        try:
            if chat_info.username:
                channel_link = f"https://t.me/{chat_info.username}"
                buttons = InlineKeyboardMarkup([[InlineKeyboardButton(f"{DOWNLOAD_BTN}", url=channel_link)]])
            else:
                invite_link = chat_info.invite_link or await bot.export_chat_invite_link(channel_to_use)
                buttons = InlineKeyboardMarkup([[InlineKeyboardButton(f"{DOWNLAOD_CHNL_BTN}", url=invite_link)]])
        except Exception as e:
...
async def _backfill_previous_episodes(name: str, aniInfo, current_ep: int, sample_url):
    ...
    try:
        ...
        # Build list of all RSS feeds to search — both low_end and normal mode
        all_rss_sources = []
        for qual, rss_url in (Var.RSS_LOW_END or {}).items():
            if rss_url:
                all_rss_sources.append(('low_end', qual, rss_url))
        for rss_url in (Var.RSS_ITEMS_ANIME or []):
            if rss_url:
                all_rss_sources.append(('normal', None, rss_url))

        seen_eps = set()
        for src_mode, qual, rss_url in all_rss_sources:
            try:
                ...
                for item in feed.entries:
                    ...
                    # Skip duplicate episode links already queued
                    if item_link in seen_eps:
                        continue
                    seen_eps.add(item_link)
                    ...
                    await rep.report(f"Backfilling episode {ep_num} of {name} via {src_mode}: {item_title}", "info")
                    await asleep(3)  # avoid flooding
                    if upload_mode == 'low_end':
                        await process_low_end(item_title, item_link, force=False)
                    else:
                        await get_animes(item_title, item_link, force=True)

            except Exception as e:
                await rep.report(f"Backfill RSS error for {name} src={src_mode}: {e}", "warning")
                continue
...
async def process_low_end(name, torrent_url, force=False):
    try:
        ...
        if is_new_anime:
            ...
            # Also check DB — if no episodes stored yet, treat as truly new
            existing_db = await db.getAnime(ani_id)
            if existing_db and any(existing_db.get(str(e)) for e in range(1, 50)):
                is_new_anime = False  # already has episodes in DB, skip backfill
        elif not force:
            return
...
async def get_or_create_anime_channel(name: str, aniInfo):
    try:
        ...
        if custom_channel:
            # Check if a side channel is also set — if so, files go to side channel
            try:
                per_anime_side = await db.get_setting(f"side_channel_{normalized_name}")
                if per_anime_side:
                    await rep.report(f"Found side channel {per_anime_side} for {base_anime_name}, using it for file uploads", "info")
                    return int(per_anime_side) if str(per_anime_side).lstrip("-").isdigit() else per_anime_side
            except Exception:
                pass
        ...
        if is_new_anime and not force:
            try:
                ...
                if total_eps and current_ep > 1:
                    ...
                    # Trigger backfill via RSS for previous episodes
                    asyncio.ensure_future(
                        _backfill_previous_episodes(name, aniInfo, current_ep, torrent)
                    )
            except Exception as _e:
                await rep.report(f"Failed to schedule backfill for {name}: {_e}", "warning")
